...
public class GridGenerator {
    ...
    private static LevelConfig[] sConfigs = new LevelConfig[] {
    ...
    private static int sLastColorSetIndex = -1;
    private static int sLastItemSetIndex = -1;

    public static void init(ReactionActivity act) {

        float gridRatio = (float)act.gameView.getWidth()/(float)act.gameView.getHeight();

        sConfigs = new LevelConfig[sConfigCount];
        for(int i = 0; i < sConfigs.length; i++) {
            sConfigs[i] = createConfig(gridRatio, i);
        }

        for(LevelConfig config : sConfigs) {
    ...
    private static LevelConfig createConfig(float ratio, int n) {
        ...
        return new LevelConfig((int)Math.pow((n+1), 2)*ADDITIONAL_POINTS, STD_ROUNDS, rows, cols,
    ...
    private static LevelConfig findConfig(int round) {
        int r = 0;
        for(int i = 0; i < sConfigs.length; i++) {
            if(sConfigs[i].numRounds + r >= round) {
                sConfigs[i].setCurrentRound(round-r);
                return sConfigs[i];
            }
            r += sConfigs[i].numRounds;
        }
        return null;
    }
    ...
    private static void fillTargets(Target[] targets, Item[][] field) {
        ...
        for (int i = 0; i < targets.length; i++) {
            int r = sRandom.nextInt(1);  //only item targets
            if(r == 0) {
                Item item;
                do {
                    item = items.get(sRandom.nextInt(items.size()));
                ...
                int color;
                do {
                    color = colors.get(sRandom.nextInt(colors.size()));
    ...
    private static void create(Item[][] field, int mode) {
        ...
        switch(mode) {
            case CREATE_1_ITEM:
                create(field, 1, itemSet[sRandom.nextInt(itemSet.length)]);
                break;
    ...
    private static void create(final Item[][] field, final int itemCount, final Item... itemCollection) {
        ...
        for (int i = 0; i < itemCount; i++) {
            Item rItem;
            do {
                int rand = sRandom.nextInt(itemCollection.length);
        ...
        for(int i = 0; i < cols; i++) {
            for(int j = 0; j < rows; j++) {
                int itemIndex;
                do {
                    itemIndex = sRandom.nextInt(pickedItems.size());
        ...
        switch(mode) {
            case COLORIZE_RANDOM:
                ...
                for(int i = 0; i < colorCount; i++) {
                    int rColor;
                    do {
                        rColor = colorSet[sRandom.nextInt(colorSet.length)];
                ...
                for(int i = 0; i < cols; i++) {
                    for(int j = 0; j < rows; j++) {
                        int colorIndex;
                        do {
                            colorIndex = sRandom.nextInt(colors.size());
    ...
    private static int[] randColorSet() {
        int dex;
        do {
            dex = sRandom.nextInt(Colorbase.colors.length);
        } while(dex == sLastColorSetIndex);
        sLastColorSetIndex = dex;
        return Colorbase.colors[dex];
    }

    private static Item[] randItemSet() {
        int dex;
        do {
            dex = sRandom.nextInt(Itembase.items.length);
        } while(dex == sLastItemSetIndex);
        sLastItemSetIndex = dex;
        return Itembase.items[dex];
    }
}
